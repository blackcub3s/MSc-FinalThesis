...
def mitjana_stack_element_wise(m): #m es un stack de ROIxROIs, perteneixent a una mateixa categoria!
	"""
	ARGUMENT INPUT: tensor -numpy ndarray- of shape (l, m, n) -> MATRIU ROIxROI for example!
	RETURN: matrix -numpy ndarray- of shape (m, n).

	EXPLANATION: Given a 3D tensor of shape input (l, m, n) this function
	Computes the arithmetic mean for each position ij of all submatrices
	of shape (m, n).

	USE: It is used in order to, given a category of patients, look at the average correlations
	in each ROI, and plot them in a ROIxROI matrix of shape (m, n) that summarizes
	intragroup medium correlations in a single array.
