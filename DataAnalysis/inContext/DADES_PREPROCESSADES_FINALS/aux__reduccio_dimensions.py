...
class Analisi_components_principals:
	...
	def __init__(self,matriu_dades,nombre_components, estandaritza_variables,llista_variables):
		...
		# I GUARDO LES MITJANES I DESVIACIONS ESTANDAR QUE FEM SERVIR PER A ESTANDARITZAR, AMB SENGLES LLISTES
		# QUE TENEN TANTS ELEMENTS COM COLUMNES DE LA MATRIU D'ENTRADA
		if estandaritza_variables:
			self.M = preprocessing.scale(self.M)
			ll_mitj_columna = []
			ll_desvest_columna = []
			for i in range(len(self.M[0])):
				ll_mitj_columna += [self.M[:,i].mean()]
				ll_desvest_columna += [self.M[:,i].std()]
			self.average = ll_mitj_columna
			self.desvest = ll_desvest_columna
		else:
			self.average = "RES"
			self.desvest = "RE"S
	...
	def puntuacions_factorials(self, imprimeix_dataset_reduit):
		...
		return dataset_reduit, self.average, self.desvest, self.A
...
#######
#X_reduit_train, ll_mitjana_train, ll_desvest_train, carregues_factorials_train = dades.puntuacions_factorials(imprimeix_dataset_reduit = False) # METODE QUE RETONRA EL DATAET REDUIT. ndarray reduit!
#######
